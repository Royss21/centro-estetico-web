  ...
  implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
  ...
  set fuseScrollbarDirectives(
    ...
    if (fuseScrollbarDirectives.length === 0) return;
  ...
  ngOnInit(): void {
    ...
    if (this.name === '') this.name = this._fuseUtilsService.randomId();
  ...
  ngAfterViewInit(): void {
    ...
    setTimeout(() => {
      ...
      if (!this._navigationContentEl) return;
      ...
      // Otherwise
        ...
        this._fuseScrollbarDirectives.forEach((fuseScrollbarDirective) => {
          // Skip if not enabled
          if (!fuseScrollbarDirective.isEnabled()) return;
  ...
  toggleAside(item: FuseNavigationItem): void {
    // Toggle
    if (this.activeAsideItemId === item.id) this.closeAside();
    else this.openAside(item);
  }
