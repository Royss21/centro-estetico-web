  ...
  implements OnInit, OnDestroy
{
  ...
  ngOnInit(): void {
    ...
    if (this._hasActiveChild(this.item, this._router.url)) this.expand();
    // Otherwise...
    else {
      // If the autoCollapse is on, collapse...
      if (this.autoCollapse) this.collapse();
    }
    ...
    this._fuseVerticalNavigationComponent.onCollapsableItemCollapsed
      ...
      .subscribe((collapsedItem) => {
        ...
        if (collapsedItem === null) return;

        // Collapse if this is a children of the collapsed item
        if (this._isChildrenOf(collapsedItem, this.item)) this.collapse();
      });
    ...
    if (this.autoCollapse) {
      this._fuseVerticalNavigationComponent.onCollapsableItemExpanded
        ...
        .subscribe((expandedItem) => {
          ...
          if (this._isChildrenOf(this.item, expandedItem)) return;

          // Check if this has a children with a matching url with the current active url
          if (this._hasActiveChild(this.item, this._router.url)) return;

          // Check if this is the expanded item
          if (this.item === expandedItem) return;
  ...
  private _hasActiveChild(
    ...
    for (const child of children) {
      if (child.children)
        if (this._hasActiveChild(child, currentUrl)) return true;
  ...
  private _isChildrenOf(
    ...
    for (const child of children)
      if (child.children) if (this._isChildrenOf(child, item)) return true;
